...
#                 # A = se3(rotz(theta), np.array([0, 0, 0])) @ se3(np.eye(3), np.array([0, 0, d])) @ se3(np.eye(3), np.array([a, 0, 0])) @ se3(rotx(alpha), np.array([0, 0, 0]))
...
#         ## copy this function into the main SerialArm class
...
#         T_tip_in_0 = self.fk(q, index=index, base=base, tip=tip)

#         # pe is the last column of the homogeneous transform matrix
...
#                 hom = self.fk(q, index=i, base=base, tip=tip)

#                 # Linear velocity for revolute is z_i_in_0 cross tip position in 0 - joint position in 0
#                 J[:3, i] = np.cross(hom[:3, 2], pe - hom[:3, 3])

#                 # Angular velocity for revolute is z_i_in_0
#                 J[3:, i] = hom[:3, 2]
...
# #         give target pose x_t and current joint position q_c (x_t is Cartesian coordinates)

# # while || x_t - x_c || > threshold

# # 	find desired direction x_d: x_t - x_c
# # 	find step direction qd: use one of two given methods (pseudo inverse or other)
# # 	q = q + qd
# # 	find x_c from our new q
# # return q

# # find step direction:
# # if pseudo-inverse
# # Jdag = J.T @ np.linalg.inv(J @ J.T + kd * eye)
# # qd = Jdag @ (x_t - x_c)
...
#         def Jdag_marc(J):
#             return J.T @ np.linalg.inv(J @ J.T + kd**2 * np.eye(6))

#         def Jdag_john(J):
#             return J.T @ np.linalg.inv(J @ J.T + kd * np.eye(6))

#         def find_error(q):
#             return target - self.fk(q)[0:3, 3]
...
#             if method == 'pinv':
#                 Jdag = Jdag_marc(J, kd)
#                 qd = Jdag @ K @ find_error(q)
                
#             elif method == 'J_T':
#                 JT = J.T
#                 qd = JT @ K @ find_error(q)
#             else:
#                 return q, False, "No method provided"
